...
public class BlinkWandItem extends WandItem {
    ...
    private void teleportPlayerToDimension(PlayerEntity player, Block block) {
        ...
        if (server == null) {
            player.sendMessage(Text.translatable("message.adm2.blink_fail", blockName), true);
            return;
        }

        ServerWorld world = Portal.getDimensionWorld(server, block);
        ...
        player.sendMessage(Text.translatable("message.adm2.blink_success", dimensionName, pos.toShortString()), true);
    }
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity player, Hand hand) {
        ...
        Block block = Portal.getRandomPortalBlock(world.getServer());
