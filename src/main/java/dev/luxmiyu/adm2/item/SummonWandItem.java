...
public class SummonWandItem extends WandItem {
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        ...
        if (savedBlock != null) {
            ...
            if (Adm2Util.isDimensionLoaded(world.getServer(), block)) {
                savedBlock = block;

                Identifier id = Registries.BLOCK.getId(block);
                String blockName = Text.translatable("block." + id.toTranslationKey()).getString();

                player.sendMessage(Adm2Util.textReplaceable("message.adm2.summon_save_success", blockName), true);
            } else {
                Identifier id = Registries.BLOCK.getId(block);
                String blockName = Text.translatable("block." + id.toTranslationKey()).getString();

                player.sendMessage(Adm2Util.textReplaceable("message.adm2.summon_save_fail", blockName), true);
            }
        }
